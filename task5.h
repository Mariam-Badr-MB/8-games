...
template <typename T>
class NumericalTic_Tac_Toe_board:public Board<T> {
public:
    NumericalTic_Tac_Toe_board();
...
template <typename T>
class  NumericalTic_Tac_Toe_player: public Player<T> {
public:
    NumericalTic_Tac_Toe_player (string name, T symbol);
...
template <typename T>
class NumericalTic_Tac_Toe_random_player : public RandomPlayer<T>{
public:
    NumericalTic_Tac_Toe_random_player(T symbol);
    void getmove(int &x=0, int &y=0) ;
};
...
NumericalTic_Tac_Toe_board<T>::NumericalTic_Tac_Toe_board()
{
...
template <typename T>
bool NumericalTic_Tac_Toe_board<T>::update_board(int x, int y, T mark) {
...
template <typename T>
void  NumericalTic_Tac_Toe_board<T>::display_board()
{
   for (int i = 0; i < this->rows; i++) {
        cout << "\n| ";
        for (int j = 0; j < this->columns; j++) {
...
template <typename T>
bool NumericalTic_Tac_Toe_board<T>::is_win()
{
     for (int i = 0; i < this->rows; i++) {
...
template <typename T>
bool NumericalTic_Tac_Toe_board<T>::is_draw() {
...
template <typename T>
bool NumericalTic_Tac_Toe_board<T>::game_is_over() {
...
template <typename T>
NumericalTic_Tac_Toe_player<T>::NumericalTic_Tac_Toe_player(string name, T symbol) : Player<T>(name, symbol) {
    this->name = name;
    this->symbol = symbol;
}
...
template <typename T>
void NumericalTic_Tac_Toe_player<T>::getmove(int& x, int& y) {
...
template <typename T>
NumericalTic_Tac_Toe_random_player<T>::NumericalTic_Tac_Toe_random_player(T symbol) : RandomPlayer<T>(symbol) {
    this->dimension = 3;
...
template <typename T>
void NumericalTic_Tac_Toe_random_player<T>::getmove(int& x, int& y) {
