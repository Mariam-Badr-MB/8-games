...
template <typename T>
class X_O_MinMax_Player3 : public Player<T> {
  _5x5_Board <T> *boardx;
public:
    X_O_MinMax_Player3(T symbol);
...
template <typename T>
X_O_MinMax_Player3<T>::X_O_MinMax_Player3(T symbol) : Player<T>(symbol) {
    this->name = "AI Player";
}

// Method to get the best move for the player
template <typename T>
void X_O_MinMax_Player3<T>::getmove(int& x, int& y) {
...
template <typename T>
int X_O_MinMax_Player3<T>::calculateMinMax(T s, bool isMaximizing) {
...
template <typename T>
std::pair<int, int> X_O_MinMax_Player3<T>::getBestMove() {
    int bestValue = std::numeric_limits<int>::min();
    std::pair<int, int> bestMove = {-1, -1};
    T opponentSymbol = (this->symbol == 'X') ? 'O' : 'X';

    //  checking the winning state
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
    ...
    // then, checking the winning of opponent
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
    ...
    //  find the best move using minmax as there is no immediate win or block,
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            if (this->boardPtr->update_board(i, j, this->symbol)) {
